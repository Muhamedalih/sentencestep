 ...
 * screen.
 */
/** Imperative handle exposed via ref — see `stop`'s own doc comment below for why this exists at all. */
export interface PronunciationButtonHandle {
  /**
   * Stops this button's own clip immediately, without touching whatever
   * source it was playing from — used by BookSentenceReader to silence the
   * sentence's narration the instant the learner clicks a word for its own
   * pronunciation (see that component's `sentenceAudioRef`). Before this,
   * the two were two entirely independent Audio elements with no shared
   * "only one voice at a time" rule, so a word click while the sentence was
   * still narrating produced two clips audibly overlapping — confirmed live
...
interface PronunciationButtonProps {
  ...
  /**
   * Fired at the very start of every play attempt (auto-play, Replay click,
   * or a Shift-triggered replay) — before this button resolves or plays
   * anything itself. BookSentenceReader uses this to stop its own word-click
   * clip first, so a sentence that starts narrating can never overlap a word
   * pronunciation the learner just triggered. Optional; every other caller
...
PronunciationButton.displayName = "PronunciationButton";
